...
public class OpenRedirectServlet extends DefaultLoginServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        ...
        if (isAccountLocked(userid)) {
            ...
            if (gotoUrl != null) {
                // Validate and encode the redirect URL to prevent open redirect.
                // Allow only relative paths that start with '/' and do not contain scheme or CRLF.
                String normalized = gotoUrl.trim();
                boolean isSafe = normalized.startsWith("/")
                ...
                if (isSafe) {
                    // Use encodeRedirectURL to properly encode session id if needed
                    res.sendRedirect(res.encodeRedirectURL(normalized));
                } else {
                    // Unsafe redirect detected; fallback to a safe internal page
                    res.sendRedirect(res.encodeRedirectURL("/admins/main"));
                }
            } else {
